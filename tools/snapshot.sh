...
[[ "$PANEL_DIR" == /* && -d "$PANEL_DIR" ]] || {
  echo "[snapshot] ✗ MEOWBOX_PANEL_DIR must be an existing absolute directory" >&2
  exit 1
}
...
if [[ -f "$DB_FILE" ]]; then
  SOURCE_DB_HASH="$(mb_hash_file_contents "$DB_FILE" "$DB_FILE-wal" "$DB_FILE-journal")"
  SOURCE_DB_FILE_HASH="$(mb_sqlite_file_fingerprint "$DB_FILE")"
  mb_sqlite_backup "$DB_FILE" "$SNAP_DIR/meowbox.db"
  DB_HASH_AFTER="$(mb_hash_file_contents "$DB_FILE" "$DB_FILE-wal" "$DB_FILE-journal")"
