 ...
 *
 * Legacy-версии (7.0/7.1/7.2/7.3) добавлены для миграции старых проектов с
 * hostPanel — они НЕ устанавливаются автоматически и помечены в /php как
 * deprecated. Использовать только для импорта существующих сайтов; новые
 * проекты на legacy-PHP — уязвимости.
 *
 * NB: PHP_MANAGED_VERSIONS в install.sh — отдельный список (что ставится
 * автоматически при первичной установке панели), и это правильно.
 * Инвариант: PHP_MANAGED_VERSIONS ⊆ SUPPORTED_PHP_VERSIONS.
 */
export const SUPPORTED_PHP_VERSIONS: string[] = (() => {
  const fromEnv = envList('SUPPORTED_PHP_VERSIONS');
  return fromEnv.length > 0
    ? fromEnv
    : ['8.4', '8.3', '8.2', '8.1', '8.0', '7.4', '7.3', '7.2', '7.1', '7.0'];
})();

/**
 * Версии PHP, считающиеся устаревшими (EOL разработчиками PHP). UI помечает
 * их предупреждением; backend позволяет ставить их только из явного запроса
 * на /php или из мастера hostpanel-миграции.
 */
export const LEGACY_PHP_VERSIONS = new Set(['7.0', '7.1', '7.2', '7.3', '7.4']);
