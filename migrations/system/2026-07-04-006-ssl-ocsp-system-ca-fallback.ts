/**
 * Обновляет nginx SSL-конфиги: если рядом с fullchain.pem нет chain.pem,
 * использует системный CA bundle для OCSP verification и добавляет resolver.
 */
...
const migration: SystemMigration = {
  ...
  description: 'Nginx SSL: OCSP fallback на системный CA bundle и resolver при отсутствии chain.pem',
  ...
  async up(ctx) {
    ...
    if (writes.length === 0) {
      ctx.log('nginx OCSP CA fallback config уже актуален');
      return;
    }
    ...
    try {
      ...
      ctx.log(`ssl OCSP CA fallback migration failed: ${(err as Error).message} — rollback`);
