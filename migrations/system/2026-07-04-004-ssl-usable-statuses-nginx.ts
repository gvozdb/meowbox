/**
 * Восстанавливает nginx HTTPS для сертификатов со статусами EXPIRING_SOON/EXPIRED.
 *
 * Эти статусы означают, что сертификат всё ещё лежит на диске и nginx может
 * его использовать. Раньше генератор считал usable только ACTIVE, из-за чего
 * домен с EXPIRING_SOON терял 443 server-блок и `10-ssl.conf`.
 */
...
const migration: SystemMigration = {
  ...
  description: 'Nginx SSL: считать EXPIRING_SOON/EXPIRED сертификаты usable для HTTPS server-блоков',
  ...
  async up(ctx) {
    ...
    for (const site of sites) {
      ...
      for (const domain of rendered.domains) {
        const sslChunk = domain.chunks['10-ssl.conf'];
        if (typeof sslChunk !== 'string') continue;
        const chunkPath = path.join(MEOWBOX_INCLUDE_DIR, site.name, domain.domainId, '10-ssl.conf');
        const currentChunk = await fs.readFile(chunkPath, 'utf8').catch(() => null);
        if (currentChunk !== sslChunk) {
          writes.push({ file: chunkPath, content: sslChunk });
        }
      }
    }

    if (ctx.dryRun) {
      ctx.log(`[dry-run] would: rewrite ${writes.length} nginx files`);
      return;
    }
    if (writes.length === 0) {
      ctx.log('nginx SSL usable status config уже актуален');
      return;
    }
    ...
    try {
      ...
      ctx.log(`ssl usable statuses migration failed: ${(err as Error).message} — rollback`);
      await restoreBackups(backups);
      throw err;
    }

    ctx.log(`OK: nginx files updated=${writes.length}`);
  },
};
